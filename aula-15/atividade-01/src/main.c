...
int search(node_t *n, int target, int current_level);
...
int main() {
    ...
    int level = search(root, target, 0);
...
int search(node_t *n, int target, int current_level) {
    ...
    if (n->type == ATOM && n->head.atom == target) {
        return current_level;
    }

    if (n->type == LIST) {
        level = search(n->head.list, target, current_level + 1);
        if (level != -1) {
            return level;
        }
    }

    level = search(n->tail, target, current_level);
    return level;
}
